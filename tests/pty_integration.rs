#![cfg(unix)]

use std::{
    collections::BTreeMap,
    fs,
    os::unix::net::UnixStream,
    path::PathBuf,
    process::{Child, Command, Stdio},
    thread,
    time::{Duration, Instant, SystemTime, UNIX_EPOCH},
};

use mult::{
    model::{ChatId, PtyKey, TerminalId},
    pty::{PtyDimensions, PtyEvent, PtyExit, PtyRuntime, PtySpawn},
};
use mult_protocol::{InstanceId, SOCKET_PATH_ENV};

/// Generous ceiling for "the thing we asked for eventually happened". Every
/// wait loop below polls and returns as soon as its condition holds, so this
/// only costs wall-clock time on a genuine failure — it must not be tightened
/// to "how long it usually takes" on an idle box.
const INTEGRATION_TIMEOUT: Duration = Duration::from_secs(30);

/// Same shape, for the daemon's socket appearing after `spawn`.
const SERVER_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);

/// The instance token every client in this file presents unless it is
/// deliberately playing a *different* `mult` instance (A3). Sharing it is what
/// models one `mult` lineage restarting and reclaiming its own panes — the
/// daemon's whole purpose — and is why the takeover test below still takes over.
const TEST_INSTANCE: InstanceId = InstanceId(0x6d75_6c74_0001);

/// A *different* `mult` installation, for the collision test.
const OTHER_INSTANCE: InstanceId = InstanceId(0x6d75_6c74_0002);

struct ServerGuard {
    child: Child,
    socket_path: PathBuf,
}

impl Drop for ServerGuard {
    fn drop(&mut self) {
        let _ = self.child.kill();
        let _ = self.child.wait();
        let _ = fs::remove_file(&self.socket_path);
    }
}

#[test]
fn client_receives_scrollback_output_and_real_exit_from_server_pty() {
    if integration_tests_are_skipped() {
        return;
    }
    let server = start_isolated_server();
    let mut runtime = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
        .expect("connect to isolated mult-server");
    let terminal = PtyKey::Terminal(TerminalId::new(7001).unwrap());

    start_short_lived_command(&mut runtime, terminal, "printf 'hello from pty\\n'; exit 7");
    let observed = wait_for_terminal_exit(&mut runtime, terminal)
        .expect("short-lived command should produce output and exit");

    assert!(
        observed.saw_scrollback,
        "server should send an initial raw scrollback replay"
    );
    assert!(observed.saw_output, "server should send raw PTY output");
    assert_eq!(observed.exit.code, 7);
    assert_eq!(observed.exit.signal, None);
    assert!(
        observed.output.contains("hello from pty"),
        "terminal output should include command stdout: {:?}",
        observed.output
    );
    assert!(!runtime.is_running(terminal));
}

#[test]
fn reconnect_replays_raw_scrollback_into_fresh_parser() {
    if integration_tests_are_skipped() {
        return;
    }
    let server = start_isolated_server();
    let terminal = PtyKey::Terminal(TerminalId::new(7003).unwrap());

    {
        let mut runtime = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
            .expect("connect to isolated mult-server");
        // Long-lived rather than `sleep 2`: the session only has to outlive the
        // detach below, and a fixed sleep turns a slow box into a false failure.
        start_short_lived_command(
            &mut runtime,
            terminal,
            "printf replayed; while true; do sleep 1; done",
        );
        wait_for_output(&mut runtime, terminal, "replayed")
            .expect("first client should receive output before detach");
    }

    let mut reconnected = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
        .expect("reconnect to isolated mult-server");
    start_short_lived_command(&mut reconnected, terminal, "printf should-not-run");
    wait_for_output(&mut reconnected, terminal, "replayed")
        .expect("reattach should replay buffered raw PTY output");
    assert!(reconnected.stop(terminal).expect("stop replayed terminal"));
}

#[test]
fn second_client_takes_over_session_from_a_still_attached_client() {
    if integration_tests_are_skipped() {
        return;
    }
    let server = start_isolated_server();
    let terminal = PtyKey::Terminal(TerminalId::new(7005).unwrap());

    // Client A attaches to a long-running session and observes its output.
    let mut client_a = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
        .expect("connect client A");
    start_short_lived_command(
        &mut client_a,
        terminal,
        "printf takeover; while true; do sleep 1; done",
    );
    wait_for_output(&mut client_a, terminal, "takeover").expect("client A should see output");

    // Client B attaches to the same session while A is *still attached*. Without
    // takeover the server rejects this as "already attached"; with it, B takes
    // over and receives the replayed scrollback. `start` reuses the existing
    // server session (CreateSession is idempotent for a known id).
    let mut client_b = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
        .expect("connect client B");
    let mut spawn = PtySpawn::command_line(
        terminal,
        "ignored-existing-session".to_string(),
        None,
        BTreeMap::new(),
    );
    spawn.size = PtyDimensions::new(6, 40);
    client_b
        .start(spawn)
        .expect("client B should take over the existing session");
    wait_for_output(&mut client_b, terminal, "takeover")
        .expect("takeover should replay buffered output to client B");

    assert!(client_b.stop(terminal).expect("stop after takeover"));
}

/// A3 + C12, end to end: two `mult` instances sharing one daemon and asking for
/// the same session number.
///
/// Session ids come from each instance's own state file, so a collision is not
/// exotic — two `mult` windows produce it immediately. The second instance used
/// to be handed the first's shell and then evict its owner, which is a full
/// takeover of whatever was running in it.
#[test]
fn a_second_instance_gets_its_own_pane_instead_of_the_first_instances() {
    if integration_tests_are_skipped() {
        return;
    }
    let server = start_isolated_server();
    let terminal = PtyKey::Terminal(TerminalId::new(7009).unwrap());

    let mut first = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
        .expect("connect the first instance");
    start_short_lived_command(
        &mut first,
        terminal,
        "printf first-instance; while true; do sleep 1; done",
    );
    wait_for_output(&mut first, terminal, "first-instance")
        .expect("the first instance sees its own output");

    let mut second = PtyRuntime::connect_to_socket(server.socket_path.clone(), OTHER_INSTANCE)
        .expect("connect the second instance");
    start_short_lived_command(
        &mut second,
        terminal,
        "printf second-instance; while true; do sleep 1; done",
    );
    wait_for_output(&mut second, terminal, "second-instance")
        .expect("the second instance gets a pane of its own");

    // The first instance was neither evicted nor shown the second's output:
    // `wait_for_output` panics on any error, which is what the eviction notice
    // would arrive as.
    wait_for_output(&mut first, terminal, "first-instance")
        .expect("the first instance is untouched");
    assert!(first.is_running(terminal));
    assert!(
        !first
            .pty_lines(terminal)
            .join("\n")
            .contains("second-instance"),
        "the two instances must not share a PTY stream"
    );

    assert!(first
        .stop(terminal)
        .expect("stop the first instance's pane"));
    assert!(second
        .stop(terminal)
        .expect("stop the second instance's pane"));
}

#[test]
fn reattach_after_server_restart_reports_vanished_session_as_exited() {
    if integration_tests_are_skipped() {
        return;
    }
    let server = start_isolated_server();
    let socket_path = server.socket_path.clone();
    let terminal = PtyKey::Terminal(TerminalId::new(7006).unwrap());

    let mut runtime = PtyRuntime::connect_to_socket(socket_path.clone(), TEST_INSTANCE)
        .expect("connect to first server");
    start_short_lived_command(
        &mut runtime,
        terminal,
        "printf alive; while true; do sleep 1; done",
    );
    wait_for_output(&mut runtime, terminal, "alive").expect("see output from first server");
    assert!(runtime.is_running(terminal));

    // Restart the daemon on the same socket path: the previous session is gone.
    drop(server);
    let _server2 = start_isolated_server_at(socket_path);

    // Nudging the runtime makes it reconnect to the fresh server and re-attach;
    // because the session no longer exists, the server answers with PaneExited
    // and the client cleanly retires the terminal instead of freezing on it.
    let status = wait_for_terminal_exit_after_reconnect(&mut runtime, terminal)
        .expect("client should learn the session vanished after a server restart");
    assert_ne!(status.code, 0, "a vanished session is not a clean exit");
    assert!(!runtime.is_running(terminal));
}

#[test]
fn terminal_is_retired_when_the_daemon_is_gone_and_not_autospawned() {
    if integration_tests_are_skipped() {
        return;
    }
    let mut server = start_isolated_server();
    let terminal = PtyKey::Terminal(TerminalId::new(7007).unwrap());

    let mut runtime = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
        .expect("connect to server");
    start_short_lived_command(
        &mut runtime,
        terminal,
        "printf live; while true; do sleep 1; done",
    );
    wait_for_output(&mut runtime, terminal, "live").expect("see output before the daemon dies");
    assert!(runtime.is_running(terminal));

    // Kill the daemon and do NOT restart it. Integration tests never autospawn
    // (the test binary is not named `mult`, so server_executable() is None), so
    // this reproduces the "daemon gone, autospawn unavailable" case.
    let _ = server.child.kill();
    let _ = server.child.wait();

    let status = wait_for_terminal_exit_after_reconnect(&mut runtime, terminal)
        .expect("client should retire the terminal once the daemon is unreachable");
    assert_eq!(
        status.signal.as_deref(),
        Some("mult-server connection lost")
    );
    assert!(!runtime.is_running(terminal));
}

#[test]
fn server_ignores_sighup_and_keeps_sessions_running() {
    if integration_tests_are_skipped() {
        return;
    }
    let mut server = start_isolated_server();
    let mut runtime = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
        .expect("connect to isolated mult-server");
    let terminal = PtyKey::Terminal(TerminalId::new(7004).unwrap());

    // The child blocks on stdin rather than on `sleep N`, so "did the PTY
    // survive the hangup" is answered by an observable marker (`after` appears
    // only once our post-SIGHUP keystroke reaches the still-live master) instead
    // of by a wall-clock race between the sleeps and the signal.
    start_short_lived_command(
        &mut runtime,
        terminal,
        "printf before; read _resume; printf after; while true; do sleep 1; done",
    );
    wait_for_output(&mut runtime, terminal, "before")
        .expect("command should produce output before hangup");

    let rc = unsafe { libc::kill(server.child.id() as i32, libc::SIGHUP) };
    assert_eq!(rc, 0, "send SIGHUP to mult-server");
    assert_server_still_running(&mut server);
    assert!(
        runtime
            .send_input(terminal, b"\n")
            .expect("send input after SIGHUP"),
        "the session should still accept input after the daemon ignored SIGHUP"
    );
    wait_for_output(&mut runtime, terminal, "after")
        .expect("server should keep PTY command running after SIGHUP");

    assert!(runtime.stop(terminal).expect("stop terminal after SIGHUP"));
}

#[test]
fn stopping_a_pane_kills_grandchildren_not_just_the_pane_shell() {
    if integration_tests_are_skipped() {
        return;
    }
    let server = start_isolated_server();
    let mut runtime = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
        .expect("connect to isolated mult-server");
    let terminal = PtyKey::Terminal(TerminalId::new(7008).unwrap());
    let pid_file = unique_pid_file_path();

    // A grandchild on the pane's terminal that ignores SIGHUP (an ignored
    // disposition survives `exec`), so the kernel's hangup on the dying session
    // leader does not reach it. Killing only the pane shell's pid therefore
    // leaves it alive holding the PTY slave open, the daemon's reader thread
    // never reaches EOF, and the thread, the master fd and the pane's retained
    // history leak for the daemon's lifetime. Only a process-group kill gets it.
    start_short_lived_command(
        &mut runtime,
        terminal,
        &format!(
            "sh -c 'trap \"\" HUP; echo $$ > {path}; exec sleep 300' & \
             printf grandchild-started; wait",
            path = pid_file.display()
        ),
    );
    wait_for_output(&mut runtime, terminal, "grandchild-started")
        .expect("the pane should report that the grandchild started");
    let grandchild = wait_for_recorded_pid(&pid_file).expect("grandchild should record its pid");
    assert!(process_exists(grandchild), "grandchild should be running");

    assert!(runtime.stop(terminal).expect("stop terminal"));

    assert!(
        wait_for_process_exit(grandchild),
        "stopping a pane must kill its whole process group, not just the pane shell"
    );
    let _ = fs::remove_file(&pid_file);
}

#[test]
fn rapid_stop_restart_and_chat_runtime_ids_keep_client_registry_consistent() {
    if integration_tests_are_skipped() {
        return;
    }
    let server = start_isolated_server();
    let mut runtime = PtyRuntime::connect_to_socket(server.socket_path.clone(), TEST_INSTANCE)
        .expect("connect to isolated mult-server");
    let terminal = PtyKey::Terminal(TerminalId::new(7002).unwrap());

    start_short_lived_command(&mut runtime, terminal, "while true; do sleep 1; done");
    assert!(runtime.is_running(terminal));
    assert!(runtime.stop(terminal).expect("stop running terminal"));
    assert!(!runtime.is_running(terminal));

    start_short_lived_command(&mut runtime, terminal, "printf restarted; exit 0");
    let restarted = wait_for_terminal_exit(&mut runtime, terminal)
        .expect("restarted command should exit naturally");
    assert_eq!(restarted.exit.code, 0);
    assert!(restarted.output.contains("restarted"));
    assert!(!runtime.is_running(terminal));

    let chat_terminal = PtyKey::ChatAgent(ChatId::new(77).unwrap());
    start_short_lived_command(&mut runtime, chat_terminal, "printf chat-agent; exit 0");
    let chat = wait_for_terminal_exit(&mut runtime, chat_terminal)
        .expect("chat-agent runtime terminal should exit naturally");
    assert_eq!(chat.exit.code, 0);
    assert!(chat.output.contains("chat-agent"));
    assert!(!runtime.is_running(chat_terminal));
}

struct ObservedTerminal {
    saw_scrollback: bool,
    saw_output: bool,
    exit: PtyExit,
    output: String,
}

fn start_short_lived_command(runtime: &mut PtyRuntime, terminal: PtyKey, command: &str) {
    let mut spawn = PtySpawn::command_line(terminal, command.to_string(), None, BTreeMap::new());
    spawn.size = PtyDimensions::new(6, 40);
    runtime.start(spawn).expect("start PTY command");
}

fn wait_for_output(runtime: &mut PtyRuntime, terminal: PtyKey, needle: &str) -> Option<()> {
    let deadline = Instant::now() + INTEGRATION_TIMEOUT;
    while Instant::now() < deadline {
        for event in runtime.drain_events() {
            if let PtyEvent::Error { message, .. } = event {
                panic!("server reported PTY error for terminal {terminal:?}: {message}");
            }
        }
        if runtime.pty_lines(terminal).join("\n").contains(needle) {
            return Some(());
        }
        thread::sleep(Duration::from_millis(20));
    }
    None
}

fn wait_for_terminal_exit(runtime: &mut PtyRuntime, terminal: PtyKey) -> Option<ObservedTerminal> {
    let deadline = Instant::now() + INTEGRATION_TIMEOUT;
    let mut saw_scrollback = false;
    let mut saw_output = false;

    while Instant::now() < deadline {
        for event in runtime.drain_events() {
            match event {
                PtyEvent::Scrollback {
                    pty: event_terminal,
                    ..
                } if event_terminal == terminal => {
                    saw_scrollback = true;
                }
                PtyEvent::Output {
                    pty: event_terminal,
                    ..
                } if event_terminal == terminal => {
                    saw_output = true;
                }
                PtyEvent::Exited {
                    pty: event_terminal,
                    status,
                } if event_terminal == terminal => {
                    return Some(ObservedTerminal {
                        saw_scrollback,
                        saw_output,
                        exit: status,
                        output: runtime.pty_lines(terminal).join("\n"),
                    });
                }
                PtyEvent::Error { message, .. } => {
                    panic!("server reported PTY error for terminal {terminal:?}: {message}");
                }
                _ => {}
            }
        }
        thread::sleep(Duration::from_millis(20));
    }

    None
}

fn wait_for_terminal_exit_after_reconnect(
    runtime: &mut PtyRuntime,
    terminal: PtyKey,
) -> Option<PtyExit> {
    let deadline = Instant::now() + INTEGRATION_TIMEOUT;
    let size = PtyDimensions::new(6, 40);
    while Instant::now() < deadline {
        // Any send forces a transparent reconnect to the restarted server and a
        // re-attach of the still-tracked terminal; errors here are expected
        // mid-reconnect and ignored.
        let _ = runtime.resize(terminal, size);
        for event in runtime.drain_events() {
            if let PtyEvent::Exited {
                pty: event_terminal,
                status,
            } = event
            {
                if event_terminal == terminal {
                    return Some(status);
                }
            }
        }
        thread::sleep(Duration::from_millis(20));
    }
    None
}

fn assert_server_still_running(server: &mut ServerGuard) {
    let deadline = Instant::now() + Duration::from_millis(500);
    while Instant::now() < deadline {
        match server.child.try_wait() {
            Ok(Some(status)) => panic!("mult-server exited after SIGHUP: {status}"),
            Ok(None) => {}
            Err(error) => panic!("failed to poll mult-server child after SIGHUP: {error}"),
        }
        thread::sleep(Duration::from_millis(20));
    }
}

/// The *only* legitimate way for these tests to not run. Everything else —
/// a missing server binary, a failed spawn, a socket that never appears — is a
/// broken harness and must fail loudly (see `start_isolated_server_at`),
/// otherwise the entire reconnect/attach/exit safety net silently evaporates.
fn integration_tests_are_skipped() -> bool {
    if std::env::var_os("MULT_SKIP_PTY_INTEGRATION").is_some() {
        eprintln!(
            "NOTICE: skipping PTY integration test because MULT_SKIP_PTY_INTEGRATION is set; \
             the daemon reconnect/attach/exit coverage did NOT run"
        );
        true
    } else {
        false
    }
}

fn start_isolated_server() -> ServerGuard {
    start_isolated_server_at(unique_socket_path())
}

fn start_isolated_server_at(socket_path: PathBuf) -> ServerGuard {
    // `env!`, not `option_env!`: cargo always exports this for an integration
    // test, so a missing binary is a build error rather than something the
    // suite can quietly skip over.
    let server_bin = env!("CARGO_BIN_EXE_mult-server");
    let mut child = Command::new(server_bin)
        .env(SOCKET_PATH_ENV, &socket_path)
        .env("SHELL", integration_test_shell())
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .unwrap_or_else(|error| {
            panic!("failed to spawn mult-server ({server_bin}) for the integration suite: {error}")
        });

    let deadline = Instant::now() + SERVER_STARTUP_TIMEOUT;
    while Instant::now() < deadline {
        match child.try_wait() {
            Ok(Some(status)) => panic!("mult-server exited before accepting connections: {status}"),
            Ok(None) => {}
            Err(error) => panic!("failed to poll mult-server child: {error}"),
        }

        if UnixStream::connect(&socket_path).is_ok() {
            return ServerGuard { child, socket_path };
        }
        thread::sleep(Duration::from_millis(20));
    }

    let _ = child.kill();
    let _ = child.wait();
    let _ = fs::remove_file(&socket_path);
    panic!(
        "mult-server did not create a usable socket at {} within {:?}; the integration suite \
         cannot run. Set MULT_SKIP_PTY_INTEGRATION=1 to deliberately skip instead.",
        socket_path.display(),
        SERVER_STARTUP_TIMEOUT
    );
}

fn integration_test_shell() -> PathBuf {
    std::env::var_os("MULT_TEST_SHELL")
        .map(PathBuf::from)
        .or_else(|| {
            let path = PathBuf::from("/bin/sh");
            path.exists().then_some(path)
        })
        .or_else(|| std::env::var_os("SHELL").map(PathBuf::from))
        .unwrap_or_else(|| PathBuf::from("sh"))
}

fn wait_for_recorded_pid(path: &PathBuf) -> Option<i32> {
    let deadline = Instant::now() + INTEGRATION_TIMEOUT;
    while Instant::now() < deadline {
        if let Ok(contents) = fs::read_to_string(path) {
            if let Ok(pid) = contents.trim().parse::<i32>() {
                return Some(pid);
            }
        }
        thread::sleep(Duration::from_millis(20));
    }
    None
}

/// `kill(pid, 0)` probes for the process without signalling it; it fails once
/// the process is gone and has been reaped by init.
fn process_exists(pid: i32) -> bool {
    unsafe { libc::kill(pid, 0) == 0 }
}

fn wait_for_process_exit(pid: i32) -> bool {
    let deadline = Instant::now() + INTEGRATION_TIMEOUT;
    while Instant::now() < deadline {
        if !process_exists(pid) {
            return true;
        }
        thread::sleep(Duration::from_millis(20));
    }
    false
}

fn unique_pid_file_path() -> PathBuf {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system clock should be after Unix epoch")
        .as_nanos();
    std::env::temp_dir().join(format!(
        "mult-pty-integration-{}-{unique}.pid",
        std::process::id()
    ))
}

fn unique_socket_path() -> PathBuf {
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system clock should be after Unix epoch")
        .as_nanos();
    std::env::temp_dir().join(format!(
        "mult-pty-integration-{}-{unique}.sock",
        std::process::id()
    ))
}
