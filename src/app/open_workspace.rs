//! The "open workspace" prompt: fuzzy-matching the configured projects, and
//! importing a directory as a workspace.
//!
//! Configured project paths are checked at the point of use rather than at
//! config load (E6): a project on an unmounted share is a normal thing to have
//! configured, so the entry is shown and marked rather than rejected.

use std::path::{Path, PathBuf};

use super::{App, ListSelection, Prompt, PromptInput};
...
pub struct OpenWorkspaceMatch {
    pub name: String,
    pub path: PathBuf,
    /// Whether `path` resolves to a directory right now.
    ///
    /// Configured project paths are checked here, at the point of use, rather
    /// than at config load (E6): a project on an unmounted share or a removable
    /// disk is a normal thing to have configured, so the entry is shown and
    /// marked instead of rejecting the config or hiding the row. Only the
    /// handful of entries the query matched are stat'ed, and only while the
    /// prompt is open.
...
impl App {
    pub fn open_workspace_matches(
        ...
        if let Some(Prompt::OpenWorkspace(prompt)) = self.mode.prompt() {
    ...
    fn move_open_workspace_selection(&mut self, delta: isize, projects: &[ConfiguredProject]) {
        let Some(Prompt::OpenWorkspace(prompt)) = self.mode.prompt() else {
            return;
        };
        if prompt.mode != OpenWorkspaceMode::ConfiguredProjects {
            return;
        }
        // Counting the matches borrows the prompt immutably, so the prompt is
        // re-matched to move the selection. That is the only reason these two
        // functions still exist; the movement itself is `ListSelection::step`.
        let len = open_workspace_matches_for(&prompt.input, projects).len();
        if let Some(Prompt::OpenWorkspace(prompt)) = self.mode.prompt_mut() {
    ...
    pub fn begin_open_workspace(&mut self, projects: &[ConfiguredProject]) {
        ...
        self.open_prompt(Prompt::OpenWorkspace(OpenWorkspacePrompt {
            ...
            fuzzy_project_score(&project.name, query).map(|score| {
                (
                    ...
                    OpenWorkspaceMatch {
                        ...
                        path_exists: expand_path(&project.path).is_dir(),
                    },
                )
            })
        })
...
mod tests {
    ...
    #[test]
    fn a_configured_project_that_is_not_on_disk_is_matched_and_marked() {
        // Marked, not hidden and not a load-time failure: the entry is still
        // the project the user meant (E6).
        let present = unique_temp_dir();
        let projects = vec![
            ConfiguredProject {
            ...
            ConfiguredProject {
                ...
                path: present.join("does-not-exist"),
            },
        ];
        let mut app = App::two_workspaces();
        app.begin_open_workspace(&projects);
        ...
        assert!(matches
            .iter()
            .any(|entry| entry.name == "present" && entry.path_exists));
        assert!(matches
            .iter()
            .any(|entry| entry.name == "absent" && !entry.path_exists));
