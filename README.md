...
Decoding is lenient field by field: a missing or `null` key takes that field's default, and an unrecognised key is ignored, so a renamed or dropped field costs you that field rather than the whole project. The leniency stops at the lists that hold your sessions — `workspaces`, `chats`, `terminals` — because defaulting one of those to empty would silently throw away exactly what it is there to protect. When a file genuinely cannot be decoded — a whole workspace or chat entry with the wrong shape, or a `null` where one of those lists belongs — it is moved aside with a `.corrupt-*` suffix and the session starts empty, and the status line then tells you where the backup went.
